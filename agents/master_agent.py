    ...
    def _initialize_sub_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                sub_agents["file_parser"] = FileParserAgent(f"parser_{self.agent_id}")
            except ImportError:
                logger.warning("FileParserAgent 不可用")
            
            # 上下文管理 Agent
            try:
                from agents.context_manager import ContextManagerAgent
                sub_agents["context_manager"] = ContextManagerAgent(f"context_{self.agent_id}")
            except ImportError:
                logger.warning("ContextManagerAgent 不可用")
            
            # 翻译协调 Agent
            try:
                from agents.translation_coordinator_agent import TranslationCoordinatorAgent
                sub_agents["translation_coordinator"] = TranslationCoordinatorAgent(f"coordinator_{self.agent_id}")
            except ImportError:
                logger.warning("TranslationCoordinatorAgent 不可用")
            
            # 质量评估 Agent
            try:
                from agents.translation_quality_evaluator import TranslationQualityEvaluator
                sub_agents["quality_evaluator"] = TranslationQualityEvaluator(f"quality_{self.agent_id}")
            except ImportError:
                logger.warning("TranslationQualityEvaluator 不可用")
            
            # 一致性检查 Agent
            try:
                from agents.consistency_checker import ConsistencyChecker
                sub_agents["consistency_checker"] = ConsistencyChecker(f"consistency_{self.agent_id}")
            except ImportError:
                logger.warning("ConsistencyChecker 不可用")
            
            # 字幕显示验证 Agent
            try:
                from agents.subtitle_display_validator import SubtitleDisplayValidator
                sub_agents["display_validator"] = SubtitleDisplayValidator(f"validator_{self.agent_id}")
            except ImportError:
                logger.warning("SubtitleDisplayValidator 不可用")
            
            # 字幕优化 Agent
            try:
                from agents.subtitle_optimization_agent import SubtitleOptimizationAgent
                sub_agents["subtitle_optimizer"] = SubtitleOptimizationAgent(f"optimizer_{self.agent_id}")
            except ImportError:
                logger.warning("SubtitleOptimizationAgent 不可用")
            
            logger.info("子 Agent 初始化完成", sub_agents_count=len(sub_agents))
            return sub_agents
            
        except Exception as e:
            logger.error("子 Agent 初始化失败", error=str(e))
            return {}
